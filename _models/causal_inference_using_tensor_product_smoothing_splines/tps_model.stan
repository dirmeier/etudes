...
data {
    ...
    vector[n_coefs + degree + 1] u_knots;
    vector[n_coefs + degree + 1] x_knots;
    vector[n_coefs + degree + 1] tr_knots;
}

transformed data {
    real xr[N_I]  = to_array_1d(X);
    real trr[N_I] = to_array_1d(tr);
    vector[n_coefs + degree + 1] knots[3];
    knots[1] = u_knots;
    knots[2] = x_knots;
    knots[3] = tr_knots;
}
...
model {
    ...
    matrix[N_I, n_coefs * n_coefs] t_design  = tensor_spline(uxr,  n_coefs, degree, knots);
    matrix[N_I, n_coefs * n_coefs * n_coefs] y_design  = tensor_spline(uxtr, n_coefs, degree, knots);
