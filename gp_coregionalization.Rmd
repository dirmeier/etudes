...
Before we model the data, we implement some utility functions. We use a exponentiated quadratic covariance function throughtout the case study, which in Jax can be implemented like this.
...
To measure the sampling time our models take, we also implement a decorator.
...
For inference, we use Numpyro's NUTS and add our previously defined decorator:
...
To model the data, we will need to choose a suitable likelihood for the domain of the proportions. A fitting one having the same support as our data $Y$ is the Beta distribution:
...
With a slight abuse of notation (and severe abuse of concepts of probability theory) the sampling statement $h_{r} \sim GP\left(0, K(\sigma^h_{r}, \rho^h_{r}), C(\omega_r)\right)$ represents a "Matrix" GP, i.e., a stochastic process of which any finite sample is a matrix normal random variable with a *fixed* number of columns. In this case the number of columns is the cardinality of ${i : i \in r}$. Or: supposing region $r$ consists of six states, then a sampling from $h_r$ yields always a matrix with six columns. 

But how do we sample from it? As usual we make use of reparameterization. We first sample
a $T \times q$ matrix of standard normals, where $T$ is the number of timepoints and $q$ is the cardinality of ${i : i \in r}$, then left multiply it with the square of the covariance matrix $ K(1, \rho^h_{r})$ (its Cholesky decomposition) while setting the covariance function's variance parameter to 1.0 (since it is not identified) and then right multiply the product with the root of the covariance $C(\omega)$.
...
As above, the diagnostics are fine: R-hats are roughly one, no divergences, and sufficiently high effective sample sizes. Next we compute the means and quantiles of the posterior predictive:
...
As before, the predictions are fine, and hardly different to the other model. Hence, lets'
