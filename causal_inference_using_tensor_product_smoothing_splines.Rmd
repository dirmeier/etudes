...
```{python}
D = pd.DataFrame({"$U$": U[i_to_o, 0], "$X$": X[:,0], "$T$": tr, "$Y$": y})
...
```{python}
@timer
def sample(model, data, iter_warmup=10, iter_sampling=30):
  return model.sample(
        data=data, 
...
model = CmdStanModel(stan_file=gp_model_file)
...
model = CmdStanModel(stan_file=gplinear_model_file)
fit = sample(model, data)
```
...
_, ax = plt.subplots(figsize=(8, 4))
sns.histplot(ite, color="#233B43", ax=ax, stat="density", bins=30)
for i in range(ite_star.shape[0]):
    sns.kdeplot(ite_star[i, :], color=palettes.discrete_qualitative_colors(3)[2], ax=ax)
plt.show()
```
...
Following @wood2006low, the constructionsof a tensor-product basis starts by constructing low-rank bases $B^V$ for every variable $V$. We then define a tensor-product spline over a set of variables as
...
x = np.linspace(-3, 2, n)
y_mean  = 2 * np.sin(x)
y = y_mean + random.normal(
  random.PRNGKey(0), shape=(n,)
) * 0.25
...
model = CmdStanModel(stan_file=tps_file)
fit = model.sample(data=data, chains=2)
y_star = np.mean(fit.draws_pd(vars="y_hat").values, axis=0)

_, ax = plt.subplots(figsize=(8, 4))
_ = ax.scatter(x, y, marker="+", color="black")
_ = ax.plot(x, y_star, color=palettes.discrete_qualitative_colors(4)[2])
plt.show()
```
...
```{python, echo=FALSE}
n = 100
x1       = np.linspace(-3, 2, n)
x2       = np.linspace(3, -2, n)
y_mean  = x1 * np.sin(x1) - np.cos(x2)
y = y_mean + random.normal(
  random.PRNGKey(0), shape=(n,)
) * 0.25
...
data = {
    "N": n,
    "X": onp.asarray(np.vstack([x1, x2])).T,
    "y": onp.asarray(y),
    "degree": 2,
    "n_knots": 6,
...
for i, (ax, x) in enumerate(zip(axes, [x1, x2])):
    _ = axes[i].scatter(x, y, marker="+", color="black")
    _ = axes[i].plot(x, y_star, color=palettes.discrete_qualitative_colors(4)[2])
plt.show()
```
